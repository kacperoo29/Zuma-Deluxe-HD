...
void Engine_Destroy() {
  ...
  for (unsigned int i = 0; i < engine.fontsLen; i++)
...
void Engine_DrawTexture(unsigned int texID, float x, float y) {
  ...
  SDL_RenderCopyF(engine.render, engine.textures[texID], NULL, &rect);
}
...
void Engine_DrawTextureWithRot(unsigned int texID, float x, float y,
  ...
  SDL_RenderCopyExF(engine.render, engine.textures[texID], NULL, &rect, angle,
                    NULL, SDL_FLIP_NONE);
}
...
void Engine_DrawTextureSDL(SDL_Texture *tex, float x, float y) {
  ...
  SDL_RenderCopyF(engine.render, tex, NULL, &rect);
}
  ...
  while (str[i] != '\0') {
    ...
    for (int j = 0; j < font->charsLen; j++) {
      if (str[i] == font->chars[j]) {
        ...
        SDL_RenderCopyF(engine.render, font->texture, &font->rectList[j],
                        &rect);
