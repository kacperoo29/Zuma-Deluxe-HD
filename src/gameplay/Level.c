      ...
      while (gID) {
        for (int i = 0; i < levelMgr.graphicsLen; i++) {
          if (strcmp(gID, levelMgr.graphics[i].id) == 0) {
            ...
            Level *lv = &levelMgr.stages[stageID]
                             .levels[levelMgr.stages[stageID].levelsLen - 1];
            lv->settingsID = NULL;
            lv->graphicsID = i;
            break;
          }
...
void Level_DrawDebug(Level *lvl) {
  ...
  for (size_t i = 0; i < lvl->spiralLen; i++) {
    ...
    SDL_RenderDrawLineF(engine.render, sx, sy,
...
char *Level_GetInfo(Level *lvl) {
  ...
  curr += snprintf(curr, end - curr, " *- Graphics -*\n");
  LevelGraphics *g = LevelMgr_GetLevelGraphics(lvl);
  curr += snprintf(curr, end - curr, " id: %s\n", g->id);
  curr += snprintf(curr, end - curr, " dispName: %s\n", g->dispName);
  curr += snprintf(curr, end - curr, " spiral: %s\n", g->spiralFile);
  curr += snprintf(curr, end - curr, " spiral2: %s\n", g->spiral2File);
  curr += snprintf(curr, end - curr, " texture: %s\n", g->textureFile);
  curr +=
      snprintf(curr, end - curr, " top texture: %s\n", g->textureTopLayerFile);
  curr += snprintf(curr, end - curr, " frog x: %lg; y: %lg\n\n", g->frogPos.x,
                   g->frogPos.y);

  curr += snprintf(curr, end - curr, "*- Settings -*\n");
  LevelSettings *s = LevelMgr_GetLevelSettings(lvl, 0);
  curr += snprintf(curr, end - curr, " id: %s\n", s->id);
  curr += snprintf(curr, end - curr, " part time: %d\n", s->partTime);
  curr += snprintf(curr, end - curr, " repeat chance: %d\n", s->repeatChance);
  curr += snprintf(curr, end - curr, " single chance: %d\n", s->singleChance);
  curr += snprintf(curr, end - curr, " slow factor: %f\n", s->slowFactor);
  curr += snprintf(curr, end - curr, " balls colors: %d\n", s->ballColors);
  curr += snprintf(curr, end - curr, " balls spd: %f\n", s->ballSpd);
  curr +=
      snprintf(curr, end - curr, " balls start count: %d\n", s->ballStartCount);
  curr += snprintf(curr, end - curr, " score: %d\n", s->gaugeScore);
