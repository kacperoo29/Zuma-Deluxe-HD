...
void BallChain_Init(BallChain *ballChain, Level *lvl, LevelSettings *settings) {
  ...
  Ball_Init(&ballChain->balls[0], 0, ballChain->startX, ballChain->startY, 0,
            randInt(0, ballChain->ballColors - 1), 0);
  ballChain->len = 1;
...
char BallChain_CollidesBack(BallChain *ballChain, int idx, int collideDist) {
  ...
  return (ballChain->balls[idx].pos - ballChain->balls[idx + 1].pos <
          collideDist);
}
...
char BallChain_CollidesFront(BallChain *ballChain, int idx, int collideDist) {
  ...
  return (ballChain->balls[idx - 1].pos - ballChain->balls[idx].pos <
          collideDist);
}
...
void BallChain_Append(BallChain *ballChain, Level *lvl,
  ...
  Ball_Init(&ballChain->balls[ballChain->len], 0, lvl->spiralStart.x,
            lvl->spiralStart.y, 0, color, 0);
  ballChain->len++;
}
          ...
          if (!ballChain->balls[i].isExploding && !ballChain->isEndReached) {
            ...
            if (end - start + 1 >= 3) {
              ...
              *score = *score + score_delta;
              ...
              float x =
                  ballChain->balls[(int)(end - ((end - start + 1) / 2))].x;
              float y =
...
void BallChain_UpdateCombos(BallChain *ballChain) {
  ...
  for (int i = 1; i < ballChain->len; i++) {
    if ((BallChain_CollidesFront(ballChain, i, BALLS_CHAIN_PAD) &&
        ...
        (!ballChain->balls[i - 1].isExploding &&
         ballChain->balls[i - 1].color != ballChain->balls[i].color))
  ...
  for (int i = 0; i < ballChain->len; i++) {
    if (ballChain->balls[i].drawPrority == priorBallsOnly &&
      ...
      if (ballChain->ballShadow.color.a > 128)
        ballChain->ballShadow.color.a = 128;
      Animation_Draw(&ballChain->ballShadow, ballChain->balls[i].x - 4,
                     ballChain->balls[i].y + 6);
    }
  }
  // Balls
  for (int i = 0; i < ballChain->len; i++) {
