...
void Frog_Draw(Frog *frog, bool introEnd) {
  ...
  if (frog->tongueShift > 0)
    frog->tongueShift -=
        FROG_REVERT_SPEED * (FROG_TONGUE_SHIFT_ON_SHOOT / FROG_SHIFT_ON_SHOOT);
  ...
  if (introEnd) {
    ...
    Animation_SetAngle(&anim, frog->rotation);
    Animation_Draw(
        &anim, frogX + frog->forwardX * (FROG_CUR_BALL_PAD - frog->tongueShift),
    ...
    anim.scale = 1.6;
    if (!frog->isShooted)
      Animation_Draw(&anim, frogX - frog->forwardX * FROG_NEXT_BALL_PAD,
                     frogY - frog->forwardY * FROG_NEXT_BALL_PAD);
  }
