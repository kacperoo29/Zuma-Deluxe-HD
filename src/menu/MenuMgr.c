...
void Button_Draw(Button *btn) {
  ...
  if (btn->withText) {
    ...
    Engine_DrawTextExt(btn->text, FONT_CANCUN_10, color, true, true, cx + dx,
                       btn->pos.y + 18 + dy);
  }
}
...
int CheckBox_IsClicked(CheckBox *cb) { return cb->clicked; }
...
void CheckBox_Draw(CheckBox *cb) {
  ...
  SDL_Rect dstRect = {cb->pos.x, cb->pos.y, CHECK_BOX_WIDTH, CHECK_BOX_HEIGHT};

  SDL_RenderCopy(engine.render, texture, &srcRect, &dstRect);
}
...
void Slider_Draw(Slider *s) {
  ...
  SDL_Rect dstSlRect = {s->pos.x + s->thumbPos, s->pos.y, 61, 49};

  SDL_RenderCopy(engine.render, texture, &srcSlRect, &dstSlRect);
}
...
void DialogueBox_Draw(DialogueBox *db) {
  ...
  for (int i = 0; i < db->sizeH; i++) {
    ...
    for (int j = 0; j < db->sizeW; j++) {
      ...
      SDL_RenderCopy(engine.render, texture, &srcBgRect, &dstBgRect);
    }
  }
  ...
  if (db->drawGodHead)
    cDy = 72;
  Engine_DrawTextExt(db->caption, FONT_NATIVE_ALIEN_EXT_18, cColor, false, true,
                     db->pos.x, dPos.y + cDy);
...
void Menu_Draw(Menu *menu) {
  ...
  if (menu->haveSky) {
    ...
    SDL_Rect dtrSky = {skyPos, 0, menu->trBgSky.w, menu->trBgSky.h};
...
void MenuMgr_Update(MenuMgr *m) {
  ...
  if (m->roomID == MR_MAIN) {
    ...
    case 3:
      ...
      break;
    case 4:
      break; /* TODO NEVER GONNA GIVE YOU UP */
    case 5:
      exit(0);
      break;
    }
...
void MenuMgr_Draw(MenuMgr *m) {
  ...
  if (m->roomID == MR_MAIN) {
    ...
    SDL_RenderCopy(engine.render, tempTex, &srcSunRect, &dstSunRect);
    ...
    SDL_RenderCopy(engine.render, tempTex, &srcGlowRect, &dstGlowRect);
    ...
    Engine_DrawText("REMASTER BY\nGALAXYSHAD AND S4LAT. 2021", FONT_CANCUN_10,
                    16, 660);
